...
import type { getProductBySlug } from "@/lib/stores/queries";

type ProductWithRelations = NonNullable<Awaited<ReturnType<typeof getProductBySlug>>>;

/**
 * Shared product detail view rendered by both the category-aware product route
 * (`/c/[category]/p/[product]`) and the legacy product route. All links and
 * structured data are category-aware so preview and canonical URLs match.
 */
export function ProductView({
  ...
  return (
    <div className="mx-auto max-w-site px-4 py-8 pb-24 sm:px-6 md:pb-8">
      ...
      <StructuredData
        data={[
          productJsonLd(store, product, galleryUrls, categorySlug),
          breadcrumbJsonLd(store, [
            { name: "Home", path: "/" },
            { name: product.category.name, path: categoryRelPath(categorySlug) },
            { name: product.title, path: productRelPath(product.slug, categorySlug) },
          ]),
          faqPageJsonLd(faq),
        ]}
      />
      ...
      <Breadcrumbs
        items={[
          { name: "Home", href: storefrontHref(store, "/") },
          {
            ...
            href: storefrontHref(store, categoryRelPath(categorySlug)),
          },
          { name: product.title },
        ]}
      />
      ...
      <div className="mt-6 grid gap-10 lg:grid-cols-2">
        <ProductGallery images={galleryUrls} media={galleryMedia} alt={product.imageAlt} />
        ...
        <div>
          ...
          <div id="purchase-options" className="mt-6 scroll-mt-24">
      ...
      {related.length > 0 && (
        <section className="mt-16" aria-labelledby="related-heading">
          ...
          <p className="mt-1 text-sm text-ink/60">
            ...
            <Link href={storefrontHref(store, "/compare")} className="text-primary underline">
              full comparison
            </Link>
            .
          </p>
          <div className="mt-5">
            <ProductGrid products={related} store={store} locale={store.locale} />
          </div>
        </section>
      )}
